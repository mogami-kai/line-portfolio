...
describe("buildClientLines（v3: 請負=数量1委託料 / 常用=人工×単価）", () => {
  ...
  it("常用(人工×単価)＋請負(数量1)を並べても二重計上せず、独立して積まれる", () => {
    ...
    // 二重計上していない＝小計(税抜)は 252000 + 300000 + 150000 = 702000。
    // 外税: 消費税=round(702000×0.1)=70200、合計(税込)=772200。
    const s = summarize(lines, 0.1);
    expect(s.subtotal).toBe(702000);
    expect(s.tax).toBe(70200);
    expect(s.exempt).toBe(0);
    expect(s.total).toBe(772200);
  });
...
describe("summarize（外税: 単価＝税抜 / 合計＝税込）", () => {
  it("小計=Σ課税明細(税抜), 消費税=round(小計×税率), exempt=立替, 合計=小計+消費税+対象外", () => {
    const s = summarize(lines, TAX);
    // 課税(税抜): 40000(常用) + 3125(残業) + 500000(請負) = 543125
    expect(s.subtotal).toBe(543125);
    expect(s.tax).toBe(54313); // round(543125×0.1)=54312.5 → 54313
    expect(s.exempt).toBe(8000); // 立替（対象外・税率0）
    // 合計（税込）＝ 小計 + 消費税 + 対象外。
    expect(s.total).toBe(543125 + 54313 + 8000);
  });

  it("立替（税率0）は小計にも消費税にも入らず、合計にだけ加算される", () => {
    const withExempt: InvoiceLine[] = [
      { sortNo: 1, itemName: "委託料", qty: 1, unitLabel: "人工", unitPrice: 20000, amount: 20000, taxRate: 0.1 },
      { sortNo: 2, itemName: "立替 パーキング", qty: 1, unitLabel: "式", unitPrice: 800, amount: 800, taxRate: 0 },
    ];
    const s = summarize(withExempt, 0.1);
    expect(s.subtotal).toBe(20000);
    expect(s.tax).toBe(2000);
    expect(s.exempt).toBe(800);
    expect(s.total).toBe(22800);
  });

  it("税率は明細のスナップショット値を使う（10%/8% 混在は区分ごとに丸める）", () => {
    const mixed: InvoiceLine[] = [
      { sortNo: 1, itemName: "委託料", qty: 1, unitLabel: "人工", unitPrice: 10005, amount: 10005, taxRate: 0.1 },
      { sortNo: 2, itemName: "軽減税率の品", qty: 1, unitLabel: "式", unitPrice: 1000, amount: 1000, taxRate: 0.08 },
    ];
    const s = summarize(mixed, 0.1);
    expect(s.subtotal).toBe(11005);
    // 10%: round(10005×0.1)=1001（1000.5→1001） / 8%: round(1000×0.08)=80
    expect(s.tax).toBe(1001 + 80);
    expect(s.total).toBe(11005 + 1081);
  });
});
...
describe("toXlsx（template_013 空テンプレ注入）", () => {
  ...
  it("宛先/振込先/明細/合計をテンプレに注入し、再読込で検証できる", async () => {
    ...
    // 外税: ご請求金額（税込）= 税抜合計 ＋ 消費税。
    const taxExcl = 140000 + 46000 + 3125; // 189125（税抜）
    expect(ws.getCell("F11").value).toBe(taxExcl + Math.round(taxExcl * 0.1));
    ...
    // 合計（外税: 小計(税抜) I50 / 消費税 K50 / 合計(税込) M50）
    const tax = Math.round(taxExcl * 0.1); // 18913（18912.5 → 四捨五入）
    expect(ws.getCell("I50").value).toBe(taxExcl);
    expect(ws.getCell("K50").value).toBe(tax);
    expect(ws.getCell("M50").value).toBe(taxExcl + tax);
    // 明細の単価セル（K列）は税抜のまま＝合計だけが税込。
    expect(ws.getCell(`K${FIRST}`).value).toBe(20000);
  ...
  it("立替（対象外・税率0）は小計に含めず合計に加算する", async () => {
    ...
    await wb2.xlsx.load(buf as ArrayBuffer);
    const ws = wb2.worksheets[0]!;
    // 外税: 小計(税抜)=20000 / 消費税=round(20000×0.1)=2000 /
    //   合計(税込)=20000＋2000＋対象外800=22800。
