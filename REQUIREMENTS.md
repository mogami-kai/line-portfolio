...
| 0.75日 | 単価 × 0.75 |
| 残業 Nh | ＋ 単価 ÷ 8 × 1.25 × N |
| 夜勤 | 人工は1日扱い。単価は下記のとおり日勤と分ける |

**夜勤の単価**（人工は 1.0 のまま、単価側で差をつける）:

| 対象 | 夜勤単価 |
|---|---|
| 従業員（職人）の給料 | 人工単価 × **1.25**（自動・`calc.nightUnit`） |
| 取引先への請求 | `Client.nightUnitPrice`（取引先ごとに設定。未設定なら日勤単価を流用） |

- 常用請求額 = Σ(人工×単価) ＋ Σ(残業h × 単価/8×1.25)、請負 = 契約金額。
- 従業員の給料 = (日勤＋半日)×人工単価 ＋ 夜勤×(人工単価×1.25) ＋ 残業h×残業単価。
...
- **税の扱いは外税**: 明細の**単価・金額はすべて税抜**。消費税は小計に上乗せし、**最終的な「合計」だけが税込**（合計 = 小計(税抜) ＋ 消費税 ＋ 対象外）。立替経費は対象外（税率0）で消費税の計算に入れない。消費税は税率区分（10%/8%）ごとに1回だけ四捨五入する。
- 税率は `InvoiceLine.taxRate` のスナップショット値を正とする（過去の請求書を現在の税率で計算し直さない）。
