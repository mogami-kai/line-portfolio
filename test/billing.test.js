...
test("lookupRate_ : exact site beats default, alias resolves to canonical, unknown -> null", () => {
  ...
  ss.__seed(SHEET.client, [
    ...
    // 取引先 "元請A" with aliases "マルエー" and "MALU".
    ["元請A", "マルエー,MALU", "末", "", "", "", ""],
  ]);
  ss.__seed(SHEET.rate, [
    HEADERS_RATE_MASTER,
    // default (現場 blank) for 元請A
    ["元請A", "", "常用", 18000, 1.25, 0, "", "既定単価"],
    // site-specific for 現場X
    ["元請A", "現場X", "常用", 20000, 1.25, 3000, "", "現場上書き"],
  ]);

  const onSite = ctx.lookupRate_("元請A", "現場X", "常用", D(3, 18));
  A.strictEqual(onSite.unit, 20000, "exact 現場 match should win over default");
  A.strictEqual(onSite.nightAdd, 3000);
...
test("buildBillingSummary_ aggregates 常用 + 請負 + 経費 and lists 単価未設定", () => {
  ...
  ss.__seed(SHEET.client, [
    HEADERS_CLIENT_MASTER,
    ["元請A", "マルエー", "末", "", "", "", ""], // 請求日ルール blank -> month end
  ]);

  // ONLY a site-specific rate for 現場X (no default), so 現場Z has no rate.
  // unit=20000, 残業係数=1.25, 夜勤割増=3000.
  ss.__seed(SHEET.rate, [
    HEADERS_RATE_MASTER,
    ["元請A", "現場X", "常用", 20000, 1.25, 3000, "", ""],
  ]);
  ...
  const unit = 20000, otFactor = 1.25, nightAdd = 3000;
  const joyoRow1 = unit * 1 + (unit / 8) * otFactor * 2 + 0;      // 20000 + 6250 = 26250
  const joyoRow2 = unit * 1 + 0 + nightAdd;                        // 20000 + 3000 = 23000
  const expectedJoyo = Math.round(joyoRow1 + joyoRow2);            // 49250
...
test("exportFreee_ emits 出面（常用）/ 請負工事一式 / 立替経費 lines with 請求日===期日", () => {
  ...
  const expectedJoyo = 20000 * 1 + (20000 / 8) * 1.25 * 2 + 20000 + 3000; // 49250
  const byItem = {};
...
test("computeBillingDate_ : default = month end, numeric 請求日ルール = that day", () => {
  ...
  ss.__seed(SHEET.client, [
    HEADERS_CLIENT_MASTER,
    ["締め末元請", "", "末", "", "", "", ""],   // no numeric rule -> month end
    ["20日締元請", "", "20", "20", "", "", ""], // 請求日ルール = "20"
  ]);
