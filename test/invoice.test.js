...
test("buildInvoiceHtml_ : 取引先・請求書番号・消費税・立替・振込先を含む", () => {
  ...
  const html = ctx.buildInvoiceHtml_(ctx.buildInvoiceModel_("2026-03", s, settings, 0), settings);
  for (const needle of ["請求書", "恵デモ", "202603-01", "消費税", "立替経費", "テスト銀行 普通 1234567", "デモ社"]) {
